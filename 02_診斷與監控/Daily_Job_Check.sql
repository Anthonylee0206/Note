/* =========================================================================
   【用途】       SQL Server Agent 所有 Job 的每日健檢：失敗、正在跑、狀態儀表板、
                  成功率趨勢、長時間跑、停用但還有排程。一次跑出 7 個結果集。
   【使用時機】   每天早上確認 Job 是否出包；Job 表現下降時找長期趨勢；交班。
   【輸入參數】   @HoursBack  （預設 24）— 失敗紀錄往回看幾小時
                  @DaysBack   （預設 30）— 成功率趨勢看幾天
                  @LongRunMin （預設 30）— 超過幾分鐘視為長時間跑
   【輸出】       7 個結果集：
                   [1] 最近失敗 Job 一覽
                   [2] 目前正在執行的 Job
                   [3] 所有 Job 狀態儀表板（失敗排最上）
                   [4] 失敗 Job 完整錯誤訊息
                   [5] N 天成功率統計（找長期不穩的）
                   [6] 長時間執行 Job（找偷跑慢的）
                   [7] 被停用但仍有排程的 Job（該清掉的殭屍）
   【風險/注意】  純查 msdb，Prod 可直接跑；SA 或有 msdb 讀權限的帳號即可。
   ========================================================================= */
